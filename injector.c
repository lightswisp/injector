...
enum log_status { LOG, WARNING, FATAL };
...
uintptr_t get_base(pid_t pid, const char *lib, bool exact) {
  ...
  while (fgets(maps_buf, sizeof(maps_buf), fd)) {
    [[maybe_unused]] int n = sscanf(maps_buf, "%lx-%lx %4s %lx %254s %d %4095s", &maps.a_s, &maps.a_e,
                                    maps.perms, &maps.offset, maps.dev, &maps.inode, maps.pathname);
...
uintptr_t align(uintptr_t len, size_t alignment) { return (len + alignment - 1) & ~(alignment - 1); }
...
ZyanU8 *generate_uninjection_shellcode(uintptr_t rsp, uintptr_t dlclose_addr, uintptr_t handle,
                                       ZyanUSize *shellcode_size) {
  // mov rdi, handle
  ...
  if (ZYAN_FAILED(ZydisEncoderEncodeInstruction(&req, blocks[current_block].encoded_instruction,
                                                &blocks[current_block].instruction_size))) {
...
ZyanU8 *generate_injection_shellcode(uintptr_t rsp, uintptr_t dlopen_addr, const char *lib,
                                     ZyanUSize *shellcode_size) {
...
bool handle_injection_trap(pid_t pid, struct user_regs_struct *old_regs, uintptr_t old_entry,
                           uint8_t *backup_buffer, size_t size) {
...
bool handle_uninjection_trap(pid_t pid, struct user_regs_struct *old_regs, uintptr_t old_entry,
                             uint8_t *backup_buffer, size_t size) {
  struct user_regs_struct regs;
  while (true) {
    int status;
...
bool inject() {
  ...
  if (external_elf_header->e_ident[0] != ELFMAG0 && external_elf_header->e_ident[1] != ELFMAG1 &&
      external_elf_header->e_ident[2] != ELFMAG2 && external_elf_header->e_ident[3] != ELFMAG3) {
  ...
  */
  ...
  dlopen_offset = (uintptr_t)dlsym(dlopen("libc.so.6", 2), "dlopen") - local_module_base;
  dlclose_offset = (uintptr_t)dlsym(dlopen("libc.so.6", 2), "dlclose") - local_module_base;
